...
router.delete('/:uid', verifyAuth, async (req: Request, res: Response) => {
    ...
    try {
        ...
        if (targetUser?.email) {
            const adapterUser = await db.collection('users').findOne({ email: targetUser.email });
            const adapterUserId = adapterUser?.id || adapterUser?._id?.toString?.();
            if (adapterUserId) matchingIds.push(adapterUserId);
        }

        if (matchingIds.length > 0) {
            await db.collection('sessions').deleteMany({ userId: { $in: matchingIds } });
            await db.collection('accounts').deleteMany({ userId: { $in: matchingIds } });
            await db.collection('verificationTokens').deleteMany({ userId: { $in: matchingIds } });
        }
