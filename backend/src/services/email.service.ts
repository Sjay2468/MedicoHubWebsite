...
export const EmailService = {
    ...
    sendWelcomeEmail: async (user: any) => {
        ...
        if (!resend || !process.env.RESEND_API_KEY || process.env.RESEND_API_KEY === 're_123') {
            ...
            // We don't throw here to avoid breaking the user flow for Welcome emails (fire-and-forget)
            // But we could if we wanted stricter checks. For now, just log.
            return;
        }
    ...
    sendMcampWelcomeEmail: async (user: any, uniqueId: string) => {
        ...
        try {
            ...
            // Don't throw for fire-and-forget
        }
    },
    ...
    sendPasswordResetEmail: async (email: string, resetLink: string) => {
        ...
        if (!resend || !process.env.RESEND_API_KEY || process.env.RESEND_API_KEY === 're_123') {
            const msg = "[EmailService] Aborting Password Reset: Missing or invalid credentials (RESEND_API_KEY)";
            console.warn(msg);
            throw new Error(msg);
        }
        ...
        try {
            ...
            throw error; // Propagate error so the API can return 500
        }
    },
    ...
    sendVerificationEmail: async (email: string, verifyLink: string) => {
        ...
        if (!resend || !process.env.RESEND_API_KEY || process.env.RESEND_API_KEY === 're_123') {
            const msg = "[EmailService] Aborting Email Verification: Missing or invalid credentials (RESEND_API_KEY)";
            console.warn(msg);
            throw new Error(msg);
        }
